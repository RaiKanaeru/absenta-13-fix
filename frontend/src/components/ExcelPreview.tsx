  ...
  return (
    <Card className={`${className}`}>
      ...
      <CardContent className="p-0">
        {/* Letterhead */}
        {showLetterhead && letterhead.enabled && letterhead.lines && letterhead.lines.length > 0 && (
          <div className="px-4 py-3 bg-white border-b border-gray-300">
            ...
            <div className={`space-y-1 ${letterhead.alignment === 'left' ? 'text-left' : letterhead.alignment === 'right' ? 'text-right' : 'text-center'}`}>
              {letterhead.lines.map((line, index) => (
                <div 
                  key={index}
                  className={index === 0 ? 'font-bold text-lg' : 'text-sm'}
                >
                  {line}
                </div>
              ))}
