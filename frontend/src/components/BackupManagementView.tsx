...
const BackupManagementView: React.FC = () => {
    ...
    const loadArchiveStats = async () => {
        try {
            const response = await fetchWithAuth('/api/admin/archive-stats');
            if (response.ok) {
                const data = await response.json();
                ...
                // Endpoint tidak tersedia, set default stats
                setArchiveStats({
                    studentRecords: 0,
                    teacherRecords: 0,
                    totalSize: 0,
            ...
            // Set default stats sebagai fallback
            setArchiveStats({
                studentRecords: 0,
                teacherRecords: 0,
                totalSize: 0,
    ...
    const loadCustomSchedules = async () => {
        try {
            const response = await fetchWithAuth('/api/admin/custom-schedules');
            if (response.ok) {
                const data = await response.json();
    ...
    const deleteCustomSchedule = async (scheduleId: string) => {
        ...
        try {
            const response = await fetchWithAuth(`/api/admin/custom-schedules/${scheduleId}`, {
                method: 'DELETE',
            });
    ...
    const runCustomSchedule = async (scheduleId: string) => {
        try {
            const response = await fetchWithAuth(`/api/admin/run-custom-schedule/${scheduleId}`, {
                method: 'POST',
            });
    ...
    const archiveOldData = async () => {
        try {
            ...
            const response = await fetchWithAuth('/api/admin/archive-old-data', {
                method: 'POST',
                body: JSON.stringify({
    ...
    const createTestArchiveData = async () => {
        try {
            setArchiveLoading(true);
            const response = await fetchWithAuth('/api/admin/create-test-archive-data', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                            ...
                            <div className="space-y-4">
                                ...
                                {backupType === 'semester' && (
                                    <>
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="backup-year">Tahun</Label>
                                            <Input 
                                                id="backup-year"
                                                type="number" 
                                                value={selectedYear} 
                                ...
                                {backupType === 'date' && (
                                    <>
                                        <div className="space-y-2">
                                            <Label htmlFor="backup-start-date">Tanggal Mulai</Label>
                                            <Input 
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="backup-end-date">Tanggal Selesai (Opsional)</Label>
                                            <Input 
                                                id="backup-end-date"
                                                type="date" 
                    ...
                    <Tabs defaultValue="general" className="space-y-4">
                        ...
                        <TabsContent value="general" className="space-y-4">
                            <Card>
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="max-backups">Max Backup yang Disimpan</Label>
                                    <Input 
                                        id="max-backups"
                                        type="number" 
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="archive-age">Usia Arsip (bulan)</Label>
                                    <Input 
                                        id="archive-age"
                                        type="number" 
                            ...
                            <Card className="border-orange-200 bg-orange-50">
                                ...
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div className="space-y-2">
                                            <Label htmlFor="custom-schedule-date">Tanggal Backup</Label>
                                            <Input 
                                                id="custom-schedule-date"
                                                type="date" 
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="custom-schedule-time">Waktu Backup</Label>
                                            <Input 
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <DialogContent className="max-w-md">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="space-y-2">
                                                <Label htmlFor="schedule-date">Tanggal Backup</Label>
                                                <Input 
                                                    id="schedule-date"
                                                    type="date" 
                                            ...
                                            <div className="space-y-2">
                                                <Label htmlFor="schedule-time">Waktu Backup</Label>
                                                <Input 
                                                    id="schedule-time"
                                                    type="time" 
