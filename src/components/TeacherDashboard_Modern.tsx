...
import { 
  ...
  Edit, XCircle, Filter, Eye, ChevronLeft, ChevronRight, Save
} from "lucide-react";
...
const AttendanceView = ({ schedule, user, onBack }: {
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      try {
        ...
        const response = await apiCall(`/api/schedule/${schedule.id}/students`);
        ...
        console.log(`✅ Processed ${data.length} students:`, data);
        setStudents(data);
        ...
        const initialTerlambat: {[key: number]: boolean} = {};
        data.forEach((student: {id: number, attendance_status?: string, attendance_note?: string, ada_tugas?: boolean, terlambat?: boolean}) => {
          ...
          // Load ada_tugas and terlambat
          initialAdaTugas[student.id] = student.ada_tugas || false;
          initialTerlambat[student.id] = student.terlambat || false;
        });
        setAttendance(initialAttendance);
        setNotes(initialNotes);
        setAdaTugas(initialAdaTugas);
        setTerlambat(initialTerlambat);
  ...
  const fetchStudentsByDate = async (tanggal: string) => {
    try {
      ...
      const response = await apiCall(`/api/schedule/${schedule.id}/students-by-date?tanggal=${tanggal}`);
      console.log(`✅ Received response for date ${tanggal}:`, response);
      ...
      console.log(`✅ Processed ${data.length} students for date ${tanggal}:`, data);
      setStudents(data);
  ...
  const toggleEditMode = () => {
    ...
    if (!isEditMode) {
      ...
      fetchStudentsByDate(new Date().toISOString().split('T')[0]);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Prepare attendance data - send only status as string (not object)
      const attendanceData: {[key: number]: AttendanceStatus} = {};
      Object.keys(attendance).forEach(studentId => {
        const studentIdNum = parseInt(studentId);
        attendanceData[studentIdNum] = attendance[studentIdNum]; // Langsung string status
      });
      ...
      const requestBody = {
        ...
        attendance: attendanceData,
        notes,
        adaTugas, // Send ada_tugas data
        terlambat, // Send terlambat data
      ...
      console.log('🔍 DEBUG: scheduleId type:', typeof schedule.id);
      console.log('🔍 DEBUG: attendanceData type:', typeof attendanceData);
      console.log('🔍 DEBUG: attendanceData keys:', Object.keys(attendanceData));
      console.log('🔍 DEBUG: notes type:', typeof notes);
      ...
      }, 1000); // Delay reload to show success message
    } catch (error) {
                  ...
                  <div className="space-y-3">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) => {
                        ...
                        // Jika user mencoba memilih status non-Hadir,
                        // reset checkbox states
                        if (newStatus !== 'Hadir') {
                          setTerlambat(prev => ({ ...prev, [student.id]: false }));
                          setAdaTugas(prev => ({ ...prev, [student.id]: false }));
                        }
                    ...
                    <div className="mt-3 flex gap-4">
                      {/* Terlambat checkbox - hanya muncul saat status Hadir */}
                        ...
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (isChecked) {
                                // Jika Terlambat dicentang, paksa status menjadi Hadir
                                setAttendance(prev => ({ ...prev, [student.id]: 'Hadir' }));
                                // Reset Ada Tugas jika Terlambat dicentang
                                setAdaTugas(prev => ({ ...prev, [student.id]: false }));
                              }
                              
                              setTerlambat(prev => ({ ...prev, [student.id]: isChecked }));
                            }}
                            className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
                          />
                          <Label htmlFor={`terlambat-${student.id}`} className="text-sm text-orange-600">
                            Terlambat
                          </Label>
                        </div>
                      )}
                      
                      {/* Ada Tugas checkbox - muncul saat status Izin, Sakit, Alpa, atau Dispen */}
                      {(attendance[student.id] === 'Izin' || attendance[student.id] === 'Sakit' || 
                        attendance[student.id] === 'Alpa' || attendance[student.id] === 'Dispen') && (
                        <div className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            ...
                            onChange={(e) => {
                              const isChecked = e.target.checked;
                              setAdaTugas(prev => ({ ...prev, [student.id]: isChecked }));
                            }}
                            className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                          />
                          <Label htmlFor={`ada-tugas-${student.id}`} className="text-sm text-blue-600">
                            Ada Tugas
                          </Label>
                        </div>
                      )}
                    ...
                    {attendance[student.id] !== 'Hadir' && (
                      <div className="mt-3">
                        <Label htmlFor={`keterangan-${student.id}`} className="text-sm font-medium text-gray-700">
                          Keterangan:
                        </Label>
                        <Textarea
...
const LaporanKehadiranSiswaView = ({ user }: { user: TeacherDashboardProps['userData'] }) => {
  ...
  const downloadExcel = async () => {
    ...
    try {
      const url = `/api/guru/download-laporan-kehadiran-siswa?kelas_id=${selectedKelas}&startDate=${startDate}&endDate=${endDate}`;
          ...
          <ExcelPreview
            ...
            data={reportData.map((student, index) => {
              const rowData: {
                ...
                total: (student.total_hadir || 0) + (student.total_izin || 0) + (student.total_sakit || 0) + (student.total_alpa || 0) + (student.total_dispen || 0),
                persentase: student.persentase_kehadiran || '0%'
              };
            ...
            columns={[
              ...
              { key: 'nama', label: 'Nama', width: 200, align: 'left' },
              { key: 'nis', label: 'NIS', width: 120, align: 'left' },
              { key: 'jenis_kelamin', label: 'L/P', width: 60, align: 'center' },
              ...
              { key: 'hadir', label: 'H', width: 60, align: 'center', format: 'number' },
              { key: 'izin', label: 'I', width: 60, align: 'center', format: 'number' },
              { key: 'sakit', label: 'S', width: 60, align: 'center', format: 'number' },
              { key: 'alpa', label: 'A', width: 60, align: 'center', format: 'number' },
              { key: 'dispen', label: 'D', width: 60, align: 'center', format: 'number' },
            ...
            reportPeriod={periode ? 
              `Periode: ${new Date(periode.startDate).toLocaleDateString('id-ID')} - ${new Date(periode.endDate).toLocaleDateString('id-ID')}` :
              'Periode: -'
            }
            showLetterhead={true}
          />
...
// Riwayat Pengajuan Izin Report View - Updated to match student page exactly
const RiwayatPengajuanIzinView = ({ user }: { user: TeacherDashboardProps['userData'] }) => {
  ...
  // Fungsi untuk toggle expand row
  const toggleRowExpansion = (pengajuanId: number) => {
    const newExpanded = new Set(expandedRows);
    if (newExpanded.has(pengajuanId)) {
      newExpanded.delete(pengajuanId);
    } else {
      newExpanded.add(pengajuanId);
    }
    setExpandedRows(newExpanded);
  };

  // Fungsi untuk handle approve/reject
  const handleApprovePengajuan = async (pengajuanId: number, action: 'disetujui' | 'ditolak', catatan: string = '') => {
    try {
      const response = await apiCall(`/api/guru/pengajuan-izin/${pengajuanId}`, {
        method: 'PUT',
        body: JSON.stringify({
          status: action,
          catatan: catatan
      ...
      if (response.success) {
        toast({
          title: "Berhasil!",
          description: `Pengajuan izin berhasil ${action === 'disetujui' ? 'disetujui' : 'ditolak'}`
        });
        // Refresh data
        fetchData();
      }
      ...
      toast({
        title: "Error",
        description: "Gagal memproses pengajuan izin",
        variant: "destructive"
      });
    }
  };
  ...
  const downloadExcel = async () => {
    try {
      ...
      const url = `/api/export/riwayat-pengajuan-izin?${params.toString()}`;
            ...
            <CardContent>
              <div className="overflow-x-auto">
                            ...
                            {izin.siswa_izin && izin.siswa_izin.length > 0 ? (
                              <div className="text-sm space-y-1">
                                {izin.siswa_izin.slice(0, 2).map((s, idx) => (
                                  <div key={idx} className="text-xs bg-gray-50 p-1 rounded break-words">
                                ...
                                {expandedRows.has(izin.id_pengajuan) && izin.siswa_izin.length > 2 && (
                                  <div className="text-xs space-y-1 mt-2 p-2 bg-gray-50 rounded border">
                                    {izin.siswa_izin.slice(2).map((s, idx) => (
                                      <div key={idx + 2} className="bg-white p-1 rounded break-words">
                                        <strong>{s.nama}:</strong> {s.alasan}
                                      </div>
                                    ))}
                                    <div 
                            ...
                            <div className="flex gap-2">
                              <Dialog>
                                ...
                                <DialogContent>
                                  ...
                                  <div className="space-y-4">
                                    <div>
                                      <p className="text-sm text-gray-600 mb-2">Pengajuan dari: <strong>{izin.nama_mapel}</strong></p>
                                      <p className="text-sm text-gray-600">Tanggal: {new Date(izin.tanggal_izin).toLocaleDateString('id-ID')}</p>
                                      <p className="text-sm text-gray-600">Siswa: {izin.total_siswa_izin || izin.siswa_izin?.length || 0} siswa</p>
                                    </div>
                                    <Textarea 
                                      placeholder="Catatan persetujuan (opsional)" 
                                      id={`approve-note-${izin.id_pengajuan}`}
                                    />
                                    <Button 
                                      onClick={() => {
                                        const textarea = document.getElementById(`approve-note-${izin.id_pengajuan}`) as HTMLTextAreaElement;
                                        handleApprovePengajuan(izin.id_pengajuan, 'disetujui', textarea.value);
                                      }}
                                ...
                                <DialogContent>
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <Textarea 
                                      ...
                                      id={`reject-note-${izin.id_pengajuan}`}
                                      required
                                    />
                                    <Button 
                                      onClick={() => {
                                        const textarea = document.getElementById(`reject-note-${izin.id_pengajuan}`) as HTMLTextAreaElement;
                                        if (textarea.value.trim()) {
                                          handleApprovePengajuan(izin.id_pengajuan, 'ditolak', textarea.value);
                                        } else {
                ...
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-gray-600">
                    Menampilkan {((currentPage - 1) * itemsPerPage) + 1} sampai {Math.min(currentPage * itemsPerPage, reportData.length)} dari {reportData.length} pengajuan
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
...
const RiwayatBandingAbsenView = ({ user }: { user: TeacherDashboardProps['userData'] }) => {
  ...
  const downloadExcel = async () => {
    try {
      ...
      const url = `/api/export/riwayat-banding-absen?${params.toString()}`;
      const response = await fetch(url, { 
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`
...
const PresensiSiswaSMKN13View = ({ user }: { user: TeacherDashboardProps['userData'] }) => {
  ...
  const downloadExcel = async () => {
    try {
      ...
      const url = `/api/export/presensi-siswa-smkn13?${params.toString()}`;
      const response = await fetch(url, { 
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`
...
const RekapKetidakhadiranView = ({ user }: { user: TeacherDashboardProps['userData'] }) => {
  ...
  const downloadExcel = async () => {
    try {
      ...
      const url = `/api/export/rekap-ketidakhadiran?${params.toString()}`;
      const response = await fetch(url, { 
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`
...
const TeacherReportsView = ({ user }: { user: TeacherDashboardProps['userData'] }) => {
  ...
  const downloadExcel = async () => {
    try {
      const params = new URLSearchParams({ startDate: dateRange.startDate, endDate: dateRange.endDate });
      if (selectedKelas && selectedKelas !== 'all') params.append('kelas_id', selectedKelas);
      const url = `/api/guru/download-attendance-excel?${params.toString()}`;
      const resp = await fetch(url, { credentials: 'include' });
      const blob = await resp.blob();
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
  ...
  const downloadSMKN13Format = async () => {
    try {
      ...
      const url = `/api/export/ringkasan-kehadiran-siswa-smkn13?${params.toString()}`;
      const response = await fetch(url, { 
        credentials: 'include',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token') || ''}`
              ...
              <TableBody>
                {pengajuanList.map((pengajuan) => (
                  <TableRow key={pengajuan.id}>
                    <TableCell>
                      {formatDateTime24(pengajuan.tanggal_pengajuan, true)}
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        <div className="text-sm font-medium">
                          {new Date(pengajuan.tanggal_mulai).toLocaleDateString('id-ID')}
                        </div>
                        <div className="text-xs text-gray-500">
                          Selesai: {new Date(pengajuan.tanggal_selesai).toLocaleDateString('id-ID')}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        <span className="font-medium">{(pengajuan as {nama_mapel?: string}).nama_mapel || 'Izin Umum'}</span>
                        <div className="text-sm text-gray-600">
                          {(pengajuan as {nama_guru?: string, jam_mulai?: string, jam_selesai?: string}).nama_guru || 'Menunggu Persetujuan'} • {(pengajuan as {jam_mulai?: string, jam_selesai?: string}).jam_mulai || 'Izin Harian'}-{(pengajuan as {jam_mulai?: string, jam_selesai?: string}).jam_selesai || 'Izin Harian'}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        {(pengajuan as {siswa_izin?: Array<{nama: string, jenis_izin: string}>, total_siswa_izin?: number}).siswa_izin && (pengajuan as {siswa_izin?: Array<{nama: string, jenis_izin: string}>}).siswa_izin.length > 0 ? (
                          <div className="space-y-1">
                            <div className="font-medium text-sm">
                              {(pengajuan as {total_siswa_izin?: number, siswa_izin?: Array<{nama: string, jenis_izin: string}>}).total_siswa_izin || (pengajuan as {siswa_izin?: Array<{nama: string, jenis_izin: string}>}).siswa_izin.length} siswa
                            </div>
                            <div className="text-xs text-gray-600 max-w-xs">
                              {(pengajuan as {siswa_izin: Array<{nama: string, jenis_izin: string}>}).siswa_izin.slice(0, 3).map((s: {nama: string, jenis_izin: string}, idx: number) => (
                                <div key={idx}>
                                  {s.nama} ({s.jenis_izin})
                                </div>
                              ))}
                              {(pengajuan as {siswa_izin?: Array<{nama: string, jenis_izin: string}>}).siswa_izin.length > 3 && (
                                <div className="text-blue-600 cursor-pointer hover:text-blue-800 hover:underline">
                                  +{(pengajuan as {siswa_izin?: Array<{nama: string, jenis_izin: string}>}).siswa_izin.length - 3} lainnya
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="space-y-1">
                            <div className="font-medium text-sm text-gray-700">
                              {pengajuan.nama_siswa}
                            </div>
                            <div className="text-xs text-gray-600">
                              NIS: {pengajuan.nis} - {pengajuan.nama_kelas}
                            </div>
                            <Badge variant="outline" className={
                              pengajuan.jenis_izin === 'sakit' ? 'bg-red-50 text-red-700' :
                              pengajuan.jenis_izin === 'izin' ? 'bg-blue-50 text-blue-700' :
                              pengajuan.jenis_izin === 'dispen' ? 'bg-purple-50 text-purple-700' :
                              'bg-gray-50 text-gray-700'
                            }>
                              {pengajuan.jenis_izin.charAt(0).toUpperCase() + pengajuan.jenis_izin.slice(1)}
                            </Badge>
                          </div>
                        )}
                      </div>
                    ...
                    <TableCell>
                      <Badge className={
                        pengajuan.status_persetujuan === 'disetujui' ? 'bg-green-100 text-green-800' :
                        pengajuan.status_persetujuan === 'ditolak' ? 'bg-red-100 text-red-800' :
                        'bg-yellow-100 text-yellow-800'
                      }>
                        {pengajuan.status_persetujuan === 'pending' ? 'Menunggu' :
                         pengajuan.status_persetujuan === 'disetujui' ? 'Disetujui' : 'Ditolak'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1 max-w-sm">
                        {(pengajuan as {siswa_izin?: Array<{nama: string, alasan: string}>}).siswa_izin && (pengajuan as {siswa_izin?: Array<{nama: string, alasan: string}>}).siswa_izin.length > 0 ? (
                          <div className="text-sm space-y-1">
                            {(pengajuan as {siswa_izin: Array<{nama: string, alasan: string}>}).siswa_izin.slice(0, 2).map((s: {nama: string, alasan: string}, idx: number) => (
                              <div key={idx} className="text-xs bg-gray-50 p-1 rounded break-words">
                                <strong>{s.nama}:</strong> {s.alasan}
                              </div>
                            ))}
                            {(pengajuan as {siswa_izin?: Array<{nama: string, alasan: string}>}).siswa_izin.length > 2 && (
                              <div className="text-xs text-blue-600 cursor-pointer hover:text-blue-800 hover:underline">
                                +{(pengajuan as {siswa_izin?: Array<{nama: string, alasan: string}>}).siswa_izin.length - 2} alasan lainnya
                              </div>
                            )}
                          </div>
                        ) : (
                          <div className="text-sm break-words">{pengajuan.alasan}</div>
                        )}
                        {pengajuan.catatan_guru && (
                          <div className="text-xs text-gray-600 bg-gray-50 p-2 rounded break-words">
                            <strong>Respon Guru:</strong> {pengajuan.catatan_guru}
                          </div>
                        )}
                      </div>
                    </TableCell>
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const processedStudents = new Set(); // Track processed students to prevent duplicates
        ...
        flat.forEach((row) => {
          ...
          // Create unique key for student to prevent duplicates
          const studentKey = `${row.nama_siswa}_${row.nis}_${row.status_kehadiran}`;
          ...
          if (!processedStudents.has(studentKey)) {
            ...
            console.log(`⚠️ Duplicate student detected: ${studentKey}`);
          }
        });
                                  ...
                                  <span className="text-sm">
                                    {(() => {
                                      try {
                                        ...
                                        const timeStr = siswa.waktu_absen;
                                        ...
                                        console.error('Error formatting time:', error, siswa.waktu_absen);
                                        return 'Invalid time';
                                      }
                                    })()}
                                  </span>
...
export const TeacherDashboard = ({ userData, onLogout }: TeacherDashboardProps) => {
  ...
  // Update currentUserData when userData changes
  useEffect(() => {
  ...
  const fetchSchedules = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        ...
        console.error('❌ Invalid schedule response format:', res);
        throw new Error('Format data jadwal tidak valid');
      }
      
      console.log('📅 Processed schedule list:', list);
