...
const ABSENTA_SCHEMA_MAPPING = {
  ...
  // Format Pengajuan Izin
  PENGAJUAN_IZIN: {
    headers: ['NO', 'TANGGAL PENGAJUAN', 'TANGGAL IZIN', 'NAMA SISWA', 'NIS', 'KELAS', 
              'JENIS IZIN', 'ALASAN', 'STATUS', 'KETERANGAN GURU', 'TANGGAL RESPON'],
    fields: {
      NO: 'row_number',
      'TANGGAL PENGAJUAN': 'tanggal_pengajuan',
...
class AbsentaExportSystem {
  ...
  async exportStudentSummary(data, dateRange) {
    try {
      ...
      this.addSchoolHeader(worksheet, {
        ...
        documentNumber: `RINGKASAN/SISWA/${new Date().getFullYear()}`,
        date: new Date().toLocaleDateString('id-ID')
      });
  ...
  async exportTeacherSummary(data, dateRange) {
    try {
      ...
      this.addSchoolHeader(worksheet, {
        ...
        subtitle: `Periode: ${dateRange.startDate} s/d ${dateRange.endDate}`,
        documentNumber: `RINGKASAN/GURU/${new Date().getFullYear()}`,
        date: new Date().toLocaleDateString('id-ID')
      });
  ...
  async exportBandingAbsen(data, dateRange) {
    try {
      ...
      this.addSchoolHeader(worksheet, {
        ...
        subtitle: `Periode: ${dateRange.startDate} s/d ${dateRange.endDate}`,
        documentNumber: `BANDING/ABSEN/${new Date().getFullYear()}`,
        date: new Date().toLocaleDateString('id-ID')
      });
  ...
  // ================================================================
  // Export Pengajuan Izin
  // ================================================================
  async exportPengajuanIzin(data, dateRange) {
    try {
      console.log('📝 Generating Pengajuan Izin Export...');
      
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('Pengajuan Izin');
      
      // Header sekolah
      this.addSchoolHeader(worksheet, {
        title: 'LAPORAN PENGAJUAN IZIN SISWA',
        subtitle: `Periode: ${dateRange.startDate} s/d ${dateRange.endDate}`,
        documentNumber: `IZIN/SISWA/${new Date().getFullYear()}`,
        date: new Date().toLocaleDateString('id-ID')
      });
      
      // Table headers
      const headerRow = 15;
      const headers = this.exportFormats.PENGAJUAN_IZIN.headers;
      
      this.addTableHeaders(worksheet, headerRow, headers);
      
      // Data rows
      let currentRow = headerRow + 1;
      data.forEach((izin, index) => {
        worksheet.getCell(currentRow, 1).value = index + 1;
        worksheet.getCell(currentRow, 2).value = izin.tanggal_pengajuan;
        worksheet.getCell(currentRow, 3).value = izin.tanggal_izin;
        worksheet.getCell(currentRow, 4).value = izin.nama_siswa;
        worksheet.getCell(currentRow, 5).value = izin.nis;
        worksheet.getCell(currentRow, 6).value = izin.nama_kelas;
        worksheet.getCell(currentRow, 7).value = izin.jenis_izin;
        worksheet.getCell(currentRow, 8).value = izin.alasan;
        worksheet.getCell(currentRow, 9).value = izin.status;
        worksheet.getCell(currentRow, 10).value = izin.keterangan_guru || '-';
        worksheet.getCell(currentRow, 11).value = izin.tanggal_respon || '-';
      ...
      // Column widths
      const columnWidths = [5, 15, 15, 25, 15, 12, 12, 30, 12, 25, 15];
      columnWidths.forEach((width, index) => {
        worksheet.getColumn(index + 1).width = width;
      });
...
NIP. 196602281997022002`;
  ...
  async exportRekapKetidakhadiranGuruSMKN13(data, options = {}) {
    ...
    this.addSchoolHeader(worksheet, {
      title: 'REKAP KETIDAKHADIRAN GURU',
      subtitle: `Tahun: ${options.tahun}`,
      documentNumber: `REKAP/GURU/SMKN13/${new Date().getFullYear()}`,
      date: new Date().toLocaleDateString('id-ID')
    });
  ...
  async exportRingkasanKehadiranSiswaSMKN13(data, options = {}) {
    ...
    this.addSchoolHeader(worksheet, {
      ...
      documentNumber: `RINGKASAN/SISWA/SMKN13/${new Date().getFullYear()}`,
      date: new Date().toLocaleDateString('id-ID')
    });
  ...
  async exportRekapKetidakhadiranSiswa(data, options = {}) {
    ...
    this.addSchoolHeader(worksheet, {
      ...
      documentNumber: `REKAP/SISWA/${new Date().getFullYear()}`,
      date: new Date().toLocaleDateString('id-ID')
    });
