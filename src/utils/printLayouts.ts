...
export const printReport = (
  ...
  switch (layout) {
    case 'simple':
      content = generateSimpleLayout(data, dateRange, reportType);
      break;
    case 'detailed':
      content = generateDetailedLayout(data, dateRange, reportType);
      break;
    case 'compact':
      content = generateCompactLayout(data, dateRange, reportType);
      break;
    case 'official':
      content = generateOfficialLayout(data, dateRange, reportType);
      break;
    default:
      content = generateSimpleLayout(data, dateRange, reportType);
  }
...
const generateSimpleLayout = (data: PrintData[], dateRange: DateRange, reportType: 'student' | 'teacher') => {
  ...
  return `
    <div class="header">
      ...
      <div class="subtitle">SMA Negeri 1 Example</div>
...
const generateDetailedLayout = (data: PrintData[], dateRange: DateRange, reportType: 'student' | 'teacher') => {
  const isStudent = reportType === 'student';
  ...
  return `
    <div class="header">
      <div class="title">${title}</div>
      <div class="subtitle">SMA Negeri 1 Example</div>
      <div class="subtitle">Jl. Pendidikan No. 123, Kota Example</div>
      <div class="date-range">Periode: ${dateRange.startDate} s/d ${dateRange.endDate}</div>
...
const generateCompactLayout = (data: PrintData[], dateRange: DateRange, reportType: 'student' | 'teacher') => {
  const isStudent = reportType === 'student';
...
const generateOfficialLayout = (data: PrintData[], dateRange: DateRange, reportType: 'student' | 'teacher') => {
  const isStudent = reportType === 'student';
  const title = isStudent ? 'LAPORAN KEHADIRAN SISWA' : 'LAPORAN KEHADIRAN GURU';
