...
NIP. 196602281997022002`;
  ...
  calculateSummary(data) {
    ...
    data.forEach(record => {
      ...
      // Hitung Terlambat (dihitung sebagai Hadir)
      if (record.terlambat && record.status === 'Hadir') {
        totalTerlambat++;
      }
      
      // Hitung Ada Tugas (dihitung sebagai Tidak Hadir)
      if (record.ada_tugas && (record.status === 'Alpa' || record.status === 'Tidak Hadir')) {
        totalAdaTugas++;
      }
    });
    ...
    return {
      ...
      ...statusCounts,
      'Terlambat (Hadir)': totalTerlambat,
      'Ada Tugas (Tidak Hadir)': totalAdaTugas
    };
  }
