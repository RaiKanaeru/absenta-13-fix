...
class DatabaseOptimization {
    constructor() {
        ...
        this.poolConfig = {
            ...this.dbConfig,
            connectionLimit: 10,        // Match main server config
        ...
        try {
            ...
            const [studentArchiveResult] = await this.pool.execute(
                `INSERT INTO absensi_siswa_archive 
                 SELECT 
                     id, siswa_id, jadwal_id, tanggal, status, keterangan, 
                     waktu_absen, guru_id, NOW() as archived_at 
                 FROM absensi_siswa 
                 WHERE tanggal < ?`,
                [cutoffDateStr]
            );
            ...
            const [teacherArchiveResult] = await this.pool.execute(
                `INSERT INTO absensi_guru_archive 
                 SELECT 
                     id_absensi, jadwal_id, guru_id, kelas_id, siswa_pencatat_id, 
                     tanggal, jam_ke, 
                     CASE 
                         WHEN status = 'Terlambat' THEN 'Hadir'
                         ELSE status 
                     END as status,
                     keterangan, waktu_catat, NOW() as archived_at 
                 FROM absensi_guru 
                 WHERE tanggal < ?`,
                [cutoffDateStr]
            );
     ...
     */
    async createTestArchiveData() {
        console.log('📦 Creating test archive data...');
        
        try {
            // Create some old test data for archiving
            const oldDate = new Date();
            oldDate.setMonth(oldDate.getMonth() - 25); // 25 months ago
            oldDate.setDate(oldDate.getDate() + Math.floor(Math.random() * 30)); // Add random day to avoid duplicates
            const oldDateStr = oldDate.toISOString().split('T')[0];
            
            // Get valid IDs from existing data
            const [students] = await this.pool.execute('SELECT id_siswa FROM siswa_perwakilan LIMIT 1');
            const [teachers] = await this.pool.execute('SELECT id_guru FROM guru LIMIT 1');
            const [classes] = await this.pool.execute('SELECT id_kelas FROM kelas LIMIT 1');
            const [schedules] = await this.pool.execute('SELECT id_jadwal FROM jadwal LIMIT 1');
            
            if (students.length === 0 || teachers.length === 0 || classes.length === 0 || schedules.length === 0) {
                throw new Error('No valid reference data found in database');
            }
            
            const studentId = students[0].id_siswa;
            const teacherId = teachers[0].id_guru;
            const classId = classes[0].id_kelas;
            const scheduleId = schedules[0].id_jadwal;
            
            // Insert test student attendance data
            const [studentResult] = await this.pool.execute(
                `INSERT INTO absensi_siswa (siswa_id, jadwal_id, tanggal, status, keterangan, waktu_absen, guru_id) 
                 VALUES (?, ?, ?, 'Hadir', 'Test data for archiving', NOW(), ?)`,
                [studentId, scheduleId, oldDateStr, teacherId]
            );
            
            // Insert test teacher attendance data
            const [teacherResult] = await this.pool.execute(
                `INSERT INTO absensi_guru (jadwal_id, guru_id, kelas_id, siswa_pencatat_id, tanggal, jam_ke, status, keterangan, waktu_catat) 
                 VALUES (?, ?, ?, ?, ?, 1, 'Hadir', 'Test data for archiving', NOW())`,
                [scheduleId, teacherId, classId, studentId, oldDateStr]
            );
            ...
            return {
                studentRecords: studentResult.affectedRows,
                teacherRecords: teacherResult.affectedRows,
                testDate: oldDateStr
            };
