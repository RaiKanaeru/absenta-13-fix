...
app.get('/api/siswa/:siswaId/pengajuan-izin', authenticateToken, requireRole(['siswa']), async (req, res) => {
    try {
        ...
        const [pengajuanRows] = await connection.execute(query, [parseInt(siswaId)]);
...
app.get('/api/siswa/:siswa_id/pengajuan-izin', authenticateToken, requireRole(['siswa']), async (req, res) => {
    try {
        const { siswa_id } = req.params;
        console.log('📋 Getting pengajuan izin kelas for siswa (legacy):', siswa_id);
        ...
        console.log('🔍 Executing query for pengajuan izin (legacy)...');
        const [pengajuanRows] = await connection.execute(query, [parseInt(siswa_id)]);
        console.log(`📊 Found ${pengajuanRows.length} pengajuan records (legacy)`);
        ...
        const pengajuanWithDetails = await Promise.all(
            pengajuanRows.map(async (pengajuan) => {
                try {
                    ...
                    console.error('❌ Error getting pengajuan detail (legacy):', detailError);
                    console.error('❌ Detail error stack (legacy):', detailError.stack);
                    return pengajuan; // Return basic pengajuan data if detail fails
                }
            })
        );

        console.log(`✅ Pengajuan izin kelas retrieved (legacy): ${pengajuanWithDetails.length} items`);
        res.json({
            success: true,
            data: pengajuanWithDetails
        });
    } catch (error) {
        console.error('❌ Error getting pengajuan izin kelas (legacy):', error);
        console.error('❌ Error stack:', error.stack);
        console.error('❌ Error details:', {
            message: error.message,
            code: error.code,
...
app.post('/api/admin/students', authenticateToken, requireRole(['admin']), async (req, res) => {
    try {
        ...
        try {
            ...
            // Insert siswa_perwakilan data
            await connection.execute(
                'INSERT INTO siswa_perwakilan (nis, nama, username, user_id, kelas_id, jabatan, jenis_kelamin, email, telepon_orangtua, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
app.put('/api/admin/students/:id', authenticateToken, requireRole(['admin']), async (req, res) => {
    try {
        ...
        const [existingNIS] = await connection.execute(
            'SELECT sp.id FROM siswa_perwakilan sp JOIN users u ON sp.user_id = u.id WHERE sp.nis = ? AND u.id != ?',
            [nis, id]
        );
        ...
        try {
            ...
            await connection.execute(
                'UPDATE siswa_perwakilan SET nama = ?, username = ?, nis = ?, kelas_id = ?, jabatan = ?, jenis_kelamin = ?, email = ?, telepon_orangtua = ?, status = ? WHERE username = ?',
